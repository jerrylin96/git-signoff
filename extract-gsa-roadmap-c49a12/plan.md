...
**Status:** Approved / Phase 1b  
**Target Worktree:** `<worktree>` (`~/.gemini/tmp/worktrees/gemini_extract-gsa-roadmap-c49a12`)  
...
3. **Accurate Empirical Statuses**: Document Pages deployed, remaining Gate 1 user action ("purchase custom domain"), live adaptive dogfood completed (`20c7120`, `2558ebc`), `verify-v1` tag verified (`f01ac253`), and PyPI `signoff-mcp` squatted on 2026-05-02.
4. **Atomic Commit Cadence**: All product file modifications (Tasks 1–3) land in a single atomic commit in Phase 2 so intermediate working states never leave git history red. Tasks 1–3 do NOT commit. Task 4 performs the single atomic commit containing the 7 product paths. Ephemeral review docs in `extract-gsa-roadmap-c49a12/` and `review_manifest_extract-gsa-roadmap-c49a12.md` are ephemeral and never merge to `main`.
...
- **RED Test Spec**:
  ...
  - Modifications:
    ...
    - Assert `skills/signoff/specs/gsa-core.md` does NOT contain:
      ...
      - `"(Stage 1a)"`
- **Expected Failure**:
  Fails at `test_skill_references.py:422` with `AssertionError: docs/roadmap.md does not exist` because `docs/roadmap.md` has not yet been created.
- **Verify Command**:
  ```bash
  pytest scripts/tests/test_skill_references.py -k test_signoff_phase3f_adaptive_intensity_contract
  ```
  *(Isolated runner: `python3 ~/.gemini/scripts/run_in_env.py <worktree> pytest scripts/tests/test_skill_references.py -k test_signoff_phase3f_adaptive_intensity_contract`)*
- **Commit Policy**: No commit (working-tree edit only).
...
- **Implementation Details**:
  ...
  - `docs/roadmap.md`:
    - Create file under MIT license with title, purpose, provenance line (`Extracted from skills/signoff/specs/gsa-core.md §6 in v3.5.0`), and link back to normative spec using exact relative path `[skills/signoff/specs/gsa-core.md](../skills/signoff/specs/gsa-core.md)`.
    ...
    - Retain open `[ ] Phase 5 (Cloud & Productionization)` with numbered gates 0–3, verified status notes (Gate 0 verified, Gate 1 Pages deployed with remaining action "purchase custom domain", Gate 2 escrow spec draft, Gate 3 deferred), `verify-v1` tag status (`f01ac253`), PyPI name squatted notice, and standing instruction to iterate `docs/productionization.md`.
    - Prune session-ephemeral instructions and qualify `scripts/sync_signoff_subtree.sh` as a dotgemini-side script.
- **Verify Command**:
  ```bash
  pytest scripts/tests/test_skill_references.py -k "test_signoff_phase3f_adaptive_intensity_contract or test_skill_folder_is_self_contained"
  ```
  *(Isolated runner: `python3 ~/.gemini/scripts/run_in_env.py <worktree> pytest scripts/tests/test_skill_references.py -k "test_signoff_phase3f_adaptive_intensity_contract or test_skill_folder_is_self_contained"`)*
- **Commit Policy**: No commit (working-tree edit only).
...
- **Verify Command**:
  ...
  ```
  *(Isolated runner: `python3 ~/.gemini/scripts/run_in_env.py <worktree> pytest scripts/tests/`)*
- **Commit Policy**: No commit (working-tree edit only).

---

### Task 4: Complete Suite Verification, Linter, Link Resolution, Commit & Manifest
Run full verification across all tests and linters, execute single atomic commit for the 7 product files, and compile review manifest.
...
- **Verify Commands**:
  ...
  ruff check .
  ```
  *(Isolated runner: `python3 ~/.gemini/scripts/run_in_env.py <worktree> pytest && python3 ~/.gemini/scripts/run_in_env.py <worktree> ruff check .`)*
...
- **Atomic Product Commit**:
  Once verification passes cleanly, stage and commit the exact 7 product paths:
  ```bash
  git add \
    skills/signoff/specs/gsa-core.md \
    docs/roadmap.md \
    ...
    scripts/recover_notes.py \
    .github/workflows/notes-recovery.yml
  git commit -m "feat: extract development roadmap from gsa-core spec into docs/roadmap.md"
  ```
