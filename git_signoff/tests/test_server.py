...
from git_signoff.server import create_server  # noqa: E402
...
def test_server_init_subcommand(monkeypatch):
    ...
    from git_signoff import server as server_mod

    monkeypatch.setattr(sys, "argv", ["git-signoff", "init", "--help"])
    with patch("git_signoff.init.main", return_value=0) as mock_init:
...
def test_server_help_flag(monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["git-signoff", "--help"])
    with pytest.raises(SystemExit) as exc:
        server_mod.main()
    assert exc.value.code == 0
    captured = capsys.readouterr()
    assert "usage: git-signoff" in captured.out
...
def test_console_scripts_declared():
    ...
    assert 'git-signoff = "git_signoff.server:main"' in text
    scripts_block = text.split("[project.scripts]")[1].split("\n[")[0]
    assert "signoff-mcp" not in scripts_block, "one command, no alias"
