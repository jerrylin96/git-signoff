...
- [x] `skills/git-signoff/SKILL.md` implements GSA v1.0 trailers, portable harness adapter resolution (`GIT_SIGNOFF_TRANSCRIPT_FILE` → `ANTIGRAVITY_CONVERSATION_ID` → `CLAUDE_CODE_SESSION_ID`), signed attestation commits, and `refs/notes/signoff` dual persistence.
