...
A reviewer working from the integration branch could not use `/git-signoff`, and three code paths decided what the integration branch is. Design, five external-review revisions, and the decisions: [`docs/attest-any-target.md`](attest-any-target.md). Implemented 2026-09-16 as `.git-signoff/config.json` + the reference precedence, target mode (`prepare --target`, `targets`, notes-then-lease commit), the preparation record, `verify-v1.5` (object-integrity rules, `--scan-refs`, head mode on every event, API eligibility in the action), the `recover` action and the adopters' notes workflow, gsa-core 3.8.0, and the component-scaled interview floor. `tag.yml` creates `verify-v1.5` and `init-v8` on merge; the release workflow cuts the version. The fetch assumption behind `scan-refs: auto` (`refs/pull/N/head` stays fetchable after branch deletion) was confirmed on this repository on 2026-09-17 and end to end by the first `notes-recovery` run after merge. A review of the merged range found three fail-closed gaps — history-mode deduplication, the 100-pull-request lookup window, and the scaffolded workflow's token permissions — fixed as `verify-v1.6` and `init-v9` (CHANGELOG).
