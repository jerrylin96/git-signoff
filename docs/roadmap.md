...
- [ ] **Pins `verify-v1.4` and `init-v7`** (2026-09-09): created by `tag.yml` on the next push to `main`. `verify-v1.4` moves the verifier into the skill folder (`skills/git-signoff/verify_signoff.py`), renames the `--audit` override to `GIT_SIGNOFF_TRANSCRIPT_FILE`, and adds the stale-pin warning; `init-v7` vendors the renamed folder with `attest.py`, scaffolds `.github/workflows/git-signoff.yml` and `.git-signoff/`, and creates the branch `git-signoff/init`. Install snippets and the scaffolded workflow already reference the new pins; they resolve once the tags exist. Then cut `v0.5.0`, the release the paper cites. (`v0.4.0` was in fact released from `49187c4` on 2026-09-09 00:02 UTC, before this branch; the paper should cite `v0.5.0`, which carries the deterministic producer.)
