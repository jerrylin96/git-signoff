...
- `Signoff-Transcript-Digest` & `Signoff-Transcript-Bytes`:
  ...
  - **Learning sessions (informative, 3.8.1):** the shipped producer supports explanation and practice without creating attestations. It refuses a recognized, session-bound `GSA-PRACTICE` assistant control event in the final transcript snapshot, including snapshot retries. Quoted examples and tool output are not events; opaque or unavailable transcripts have only the skill-level separation rule. Explanation can precede a fresh real interview. This is a producer workflow convention, not a trailer or a change to status derivation or verifier evidence rules.
